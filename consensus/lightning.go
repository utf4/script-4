...
func (g *LightningEngine) isLightning() bool {
	logger.Debugf("DEBUG309REWARDS 		signerIndex=%d", g.signerIndex)
...
func (g *LightningEngine) StartNewBlock(block common.Hash) {
	...
	logger.WithFields(log.Fields{
		...
		"index":      gcp.WithStake().Index(g.privKey.PublicKey()),
	...
	logger.Debugf("DEBUG309REWARDS 		signerIndex=%d before isLightning check", g.signerIndex)
	logger.Debugf("DEBUG309REWARDS 		isLightning result=%v", g.isLightning())
